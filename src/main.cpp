...
void loop() {
  ...
  // Lire capteurs et publier toutes les 2s
  if (millis() - lastSend >= 2000) {
    ...
    // Calcul niveau alerte — fuite désactivée temporairement
    niveauGlobal = "NORMAL";
    if (valGaz > SEUIL_CRITIQUE) niveauGlobal = "CRITIQUE";
    else if (valGaz > SEUIL_DANGER) niveauGlobal = "DANGER";
    else if (valGaz > SEUIL_WARNING) niveauGlobal = "WARNING";

    // Construire et publier JSON
    char jsonBuffer[256];
    snprintf(jsonBuffer, sizeof(jsonBuffer),
      ...
      valGaz, valCO, temp, hum, niveau_cuve,
      niveauGlobal.c_str()
    );

    client.publish(topic_json, jsonBuffer);
    client.publish(topic_alerte, niveauGlobal.c_str());
    Serial.println(jsonBuffer);
    Serial.print("Niveau alerte: ");
    Serial.println(niveauGlobal);
  }
  ...
  // Gérer LED et Buzzer en continu
  gererLED(niveauGlobal);
  gererBuzzer(niveauGlobal);
}